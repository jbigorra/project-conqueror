...
/**
 * Parses a TFS (Team Foundation Server) changeset log string into an array of VCS entries.
 *
 * Expects the log to have been generated by `tf history /format:detailed` in EN-US locale.
 * Each entry represents one file item changed in one changeset. Changesets are separated
 * by a line of dashes. Date is parsed from EN-US long format (e.g. "Thursday, July 23, 2015 4:34:31 PM").
 *
 * @param text - Raw log text from a TFS `tf history /format:detailed` output.
...
/**
 * Reads a TFS log file from disk and parses it into VCS entries.
 *
 * Convenience async wrapper around {@link parseReadLog}.
 *
 * @param filePath - Absolute or relative path to the TFS log file.
 * @param options - Reserved for future use; pass `{}`.
 * @returns Promise resolving to an array of `TfsEntry` objects.
 *
 * @example
 * const entries = await parseLog("path/to/tfs.log", {});
 * // [{ author: "Ryan Coy", entity: "/Project", rev: "5", date: "2015-07-23", message: "Created team project folder /Project via the Team Project Creation Wizard" }]
 */
