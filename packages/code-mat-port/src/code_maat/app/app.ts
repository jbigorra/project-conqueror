...
function runAnalysisOn(entries: VCSEntry[], options: AppOptions): unknown[] {
  switch (options.analysis) {
    ...
    case "messages":             return commitMessages.byWordFrequency(entries, { expressionToMatch: options.expressionToMatch ?? "" });
    case "age":                  return codeAge.byAge(entries, options.ageTimeNow);
    default:
      throw new Error(`Invalid analysis requested: ${options.analysis}. Supported analyses are: authors, revisions, coupling, soc, summary, identity, abs-churn, author-churn, entity-churn, entity-ownership, main-dev, refactoring-main-dev, entity-effort, main-dev-by-revs, fragmentation, communication, messages, age.`);
  }
}
