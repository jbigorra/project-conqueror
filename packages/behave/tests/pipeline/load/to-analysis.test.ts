...
describe("toAnalysis", () => {
  ...
  test("wraps data as CSV string when format is csv", async () => {
    const data = [{ entity: "foo.ts", nRevs: 10 }]
    const result = await Effect.runPromise(toAnalysis("revisions", data, { format: "csv" }))
  ...
  test("extracts parameters stripping format field", async () => {
    ...
    const result = await Effect.runPromise(toAnalysis("revisions", data, input))
    expect(result.metadata.parameters).not.toHaveProperty("format")
    expect(result.metadata.parameters).toEqual({ minRevs: 5, gitLogPath: "/log" })
  })
