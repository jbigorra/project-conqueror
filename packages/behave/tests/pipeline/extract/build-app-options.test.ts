...
describe("buildAppOptions", () => {
  ...
  test("forwards analysis-specific fields", () => {
    const input: SimpleAnalysisInput = { gitLogPath: "/log", ageTimeNow: "2026-03-20", expressionToMatch: "fix.*", group: "group-spec", teamMapFile: "/teams.csv", temporalPeriod: "30" }
  ...
  test("merges partial options with defaults", () => {
    const input: SimpleAnalysisInput = { gitLogPath: "/log", options: { minRevs: 10 } }
    const result = buildAppOptions("revisions", input)
