...
describe("withDefaults", () => {
  test("returns all defaults when no options provided", () => {
    ...
    expect(result).toEqual({ minRevs: 5, minSharedRevs: 5, minCoupling: 30, maxCoupling: 100, maxChangesetSize: 30 })
  })
  ...
  test("overrides all fields", () => {
    const result = withDefaults({ minRevs: 1, minSharedRevs: 1, minCoupling: 0, maxCoupling: 50, maxChangesetSize: 100 })
