...
describe("revisions analysis", () => {
  const cannedData = [{ entity: "foo.ts", nRevs: 10 }, { entity: "bar.ts", nRevs: 5 }]
  ...
  test("passes options through to CodeMaatService", async () => {
    ...
    await Effect.runPromise(
      revisionsEffect({ gitLogPath: "/path/log", vcsType: "git2", options: { minRevs: 10 } }).pipe(Effect.provide(SpyLayer))
    )
    expect(capturedOptions).toEqual(expect.objectContaining({
      analysis: "revisions", versionControl: "git2", minRevs: 10, minSharedRevs: 5,
    }))
  })
})
