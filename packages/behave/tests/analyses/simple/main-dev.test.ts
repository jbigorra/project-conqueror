...
import { mainDevEffect } from "../../../src/analyses/simple/main-dev"
import { CodeMaatService } from "../../../src/services/code-maat"

describe("main-dev analysis", () => {
  const cannedData = [{ entity: "foo.ts", mainDev: "alice", added: 80, totalAdded: 100, ownership: 0.8 }]
  ...
  test("returns Analysis with JSON data by default", async () => {
    const result = await Effect.runPromise(
      mainDevEffect({ gitLogPath: "/path/log" }).pipe(Effect.provide(TestLayer))
    )
  ...
  test("returns Analysis with CSV data when format=csv", async () => {
    const result = await Effect.runPromise(
      mainDevEffect({ gitLogPath: "/path/log", format: "csv" }).pipe(Effect.provide(TestLayer))
    )
  ...
  test("passes options through to CodeMaatService", async () => {
    ...
    await Effect.runPromise(
      mainDevEffect({ gitLogPath: "/path/log", vcsType: "git2" }).pipe(Effect.provide(SpyLayer))
    )
    expect(capturedOptions).toEqual(expect.objectContaining({ analysis: "main-dev", versionControl: "git2" }))
  })
})
