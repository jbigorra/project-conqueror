...
describe("abs-churn analysis", () => {
	const cannedData = [
		{ date: "2026-01-01", added: 100, deleted: 20, commits: 5 },
	];
	...
	test("returns Analysis with JSON data by default", async () => {
		const result = await Effect.runPromise(
			absChurnEffect({ gitLogPath: "/path/log" }).pipe(
	...
	test("returns Analysis with CSV data when format=csv", async () => {
		const result = await Effect.runPromise(
			absChurnEffect({ gitLogPath: "/path/log", format: "csv" }).pipe(
				Effect.provide(TestLayer),
			),
		);
	...
	test("passes options through to CodeMaatService", async () => {
		...
		await Effect.runPromise(
			absChurnEffect({ gitLogPath: "/path/log", vcsType: "git2" }).pipe(
