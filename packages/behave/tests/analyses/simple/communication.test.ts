...
describe("communication analysis", () => {
  ...
  test("returns Analysis with CSV data when format=csv", async () => {
    const result = await Effect.runPromise(
      communicationEffect({ gitLogPath: "/path/log", format: "csv" }).pipe(Effect.provide(TestLayer))
    )
  ...
  test("passes options through to CodeMaatService", async () => {
    ...
    await Effect.runPromise(
      communicationEffect({ gitLogPath: "/path/log", vcsType: "git2" }).pipe(Effect.provide(SpyLayer))
    )
    expect(capturedOptions).toEqual(expect.objectContaining({ analysis: "communication", versionControl: "git2" }))
  })
})
