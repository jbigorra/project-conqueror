...
describe("fragmentation analysis", () => {
  ...
  test("returns Analysis with JSON data by default", async () => {
    const result = await Effect.runPromise(
      fragmentationEffect({ gitLogPath: "/path/log" }).pipe(Effect.provide(TestLayer))
    )
  ...
  test("returns Analysis with CSV data when format=csv", async () => {
    const result = await Effect.runPromise(
      fragmentationEffect({ gitLogPath: "/path/log", format: "csv" }).pipe(Effect.provide(TestLayer))
    )
  ...
  test("passes options through to CodeMaatService", async () => {
    ...
    await Effect.runPromise(
      fragmentationEffect({ gitLogPath: "/path/log", vcsType: "git2" }).pipe(Effect.provide(SpyLayer))
    )
    expect(capturedOptions).toEqual(expect.objectContaining({ analysis: "fragmentation", versionControl: "git2" }))
  })
})
