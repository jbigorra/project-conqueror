...
describe("entity-effort analysis", () => {
	const cannedData = [
		{ entity: "foo.ts", author: "alice", authorRevs: 8, totalRevs: 10 },
	];
	...
	test("returns Analysis with JSON data by default", async () => {
		const result = await Effect.runPromise(
			entityEffortEffect({ gitLogPath: "/path/log" }).pipe(
	...
	test("returns Analysis with CSV data when format=csv", async () => {
		const result = await Effect.runPromise(
			entityEffortEffect({ gitLogPath: "/path/log", format: "csv" }).pipe(
				Effect.provide(TestLayer),
			),
		);
	...
	test("passes options through to CodeMaatService", async () => {
		...
		await Effect.runPromise(
			entityEffortEffect({ gitLogPath: "/path/log", vcsType: "git2" }).pipe(
