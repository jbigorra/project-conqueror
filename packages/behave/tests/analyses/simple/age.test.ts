...
describe("age analysis", () => {
  ...
  test("returns Analysis with JSON data by default", async () => {
    const result = await Effect.runPromise(
      ageEffect({ gitLogPath: "/path/log", ageTimeNow: "2026-01-01" }).pipe(Effect.provide(TestLayer))
    )
  ...
  test("returns Analysis with CSV data when format=csv", async () => {
    const result = await Effect.runPromise(
      ageEffect({ gitLogPath: "/path/log", ageTimeNow: "2026-01-01", format: "csv" }).pipe(Effect.provide(TestLayer))
    )
  ...
  test("passes options through to CodeMaatService", async () => {
    ...
    await Effect.runPromise(
      ageEffect({ gitLogPath: "/path/log", ageTimeNow: "2026-01-01", vcsType: "git2" }).pipe(Effect.provide(SpyLayer))
    )
    expect(capturedOptions).toEqual(expect.objectContaining({ analysis: "age", versionControl: "git2" }))
  })
