...
import { entityChurnEffect } from "../../../src/analyses/simple/entity-churn"
import { CodeMaatService } from "../../../src/services/code-maat"

describe("entity-churn analysis", () => {
  const cannedData = [{ entity: "foo.ts", added: 100, deleted: 20, commits: 5 }]
  ...
  test("returns Analysis with JSON data by default", async () => {
    const result = await Effect.runPromise(
      entityChurnEffect({ gitLogPath: "/path/log" }).pipe(Effect.provide(TestLayer))
    )
  ...
  test("returns Analysis with CSV data when format=csv", async () => {
    const result = await Effect.runPromise(
      entityChurnEffect({ gitLogPath: "/path/log", format: "csv" }).pipe(Effect.provide(TestLayer))
    )
  ...
  test("passes options through to CodeMaatService", async () => {
    ...
    await Effect.runPromise(
      entityChurnEffect({ gitLogPath: "/path/log", vcsType: "git2" }).pipe(Effect.provide(SpyLayer))
    )
    expect(capturedOptions).toEqual(expect.objectContaining({ analysis: "entity-churn", versionControl: "git2" }))
  })
})
