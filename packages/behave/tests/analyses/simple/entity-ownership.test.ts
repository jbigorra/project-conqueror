...
import { entityOwnershipEffect } from "../../../src/analyses/simple/entity-ownership"
import { CodeMaatService } from "../../../src/services/code-maat"

describe("entity-ownership analysis", () => {
  const cannedData = [{ entity: "foo.ts", author: "alice", added: 80, deleted: 10 }]
  ...
  test("returns Analysis with JSON data by default", async () => {
    const result = await Effect.runPromise(
      entityOwnershipEffect({ gitLogPath: "/path/log" }).pipe(Effect.provide(TestLayer))
    )
  ...
  test("returns Analysis with CSV data when format=csv", async () => {
    const result = await Effect.runPromise(
      entityOwnershipEffect({ gitLogPath: "/path/log", format: "csv" }).pipe(Effect.provide(TestLayer))
    )
  ...
  test("passes options through to CodeMaatService", async () => {
    ...
    await Effect.runPromise(
      entityOwnershipEffect({ gitLogPath: "/path/log", vcsType: "git2" }).pipe(Effect.provide(SpyLayer))
    )
    expect(capturedOptions).toEqual(expect.objectContaining({ analysis: "entity-ownership", versionControl: "git2" }))
  })
})
